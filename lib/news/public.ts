...
export async function getPublishedArticles(limit = 24) {
  ...
  const { data, error } = await supabase
    .from("articles")
    .select("id,slug,title,subtitle,excerpt,category,featured_image_url,seo_title,seo_description,canonical_url,published_at,updated_at,created_at,authors(name,slug,bio,avatar_url)")
    .eq("status", "published")

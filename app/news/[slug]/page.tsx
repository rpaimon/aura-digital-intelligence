...
import { StoryCard, StoryVisual, CategoryChip } from "@/components/public-news/framagz-ui";
...
import { articlePath, auraServiceForArticle, canonicalArticleUrl, displayCategory, getPublishedArticleBySlug, getRelatedArticles, getPublishedArticles, publicSlugForArticle, readingTime, siteUrl, stripSourcesSection } from "@/lib/news/public";
...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const shareImage = `${siteUrl()}/social-card/${encodeURIComponent(publicSlugForArticle(article))}`;
...
export default async function ArticlePage({ params }: { params: Params }) {
  ...
  const latest = await getPublishedArticles(6);
  ...
  return (
    <main className="adi-public-shell min-h-screen">
      ...
      <article>
        <header className="border-b border-white/10">
          <div className="mx-auto max-w-[1180px] px-4 py-7 sm:px-6 lg:px-8 lg:py-10">
            ...
            <div className="mt-7 max-w-[860px]">
              ...
              <h1 className="adi-article-title adi-title-balance mt-5">{article.title}</h1>
              {article.subtitle && <p className="mt-4 max-w-3xl text-[1.05rem] font-semibold leading-7 text-white/54 sm:text-lg sm:leading-8">{article.subtitle}</p>}
            </div>
        ...
        <div className="mx-auto max-w-[1180px] sm:px-6 lg:px-8">
          <div className="adi-story-media sm:mt-6">
            <StoryVisual article={article} className="adi-mobile-full aspect-[16/9] max-h-[560px]" />
          </div>
        ...
        <section className="adi-reading-surface mt-0 sm:mt-8">
          <div className="mx-auto grid max-w-[1180px] gap-10 px-4 py-8 sm:px-6 lg:grid-cols-[minmax(0,760px)_300px] lg:justify-between lg:px-8 lg:py-11">
            <div className="min-w-0">
              {article.excerpt && <p className="adi-reading-copy max-w-[68ch] border-b pb-6 text-[1.12rem] font-black leading-[1.5] tracking-[-0.02em] sm:text-[1.35rem]" style={{ borderColor: "var(--adi-reading-line)" }}>{article.excerpt}</p>}
            ...
            <aside className="adi-article-sidebar space-y-7 border-t pt-7 lg:border-l lg:border-t-0 lg:pl-7 lg:pt-0" style={{ borderColor: "var(--adi-reading-line)" }}>
              ...
              <div className="adi-sidebar-module">
                <p className="text-[9px] font-black uppercase tracking-[0.18em] opacity-45">Share this intelligence</p>
                <p className="adi-reading-copy mt-2 text-sm leading-6">Useful to a colleague or business owner? Send the article directly from the share controls above.</p>
              </div>
            </aside>
          </div>
        </section>
      ...
      {related.length > 0 && <section className="border-t border-white/10"><div className="mx-auto max-w-[1180px] px-0 py-9 sm:px-6 lg:px-8 lg:py-12"><div className="mb-5 flex items-end justify-between gap-4 px-4 sm:px-0"><div><p className="adi-kicker">Continue reading</p><h2 className="adi-section-title mt-1">Related intelligence</h2></div><Link href="/news" className="text-[10px] font-black uppercase tracking-[0.12em] text-white/45 hover:text-white">All news →</Link></div><div className="adi-story-grid three">{related.map((item)=><StoryCard key={item.id} article={item} compact />)}</div></div></section>}

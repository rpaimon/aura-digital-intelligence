...
export function NewsHeader() {
  return (
    <header className="sticky top-0 z-50 border-b border-white/10 bg-[#07090d]/95 backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1480px] items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-2">
          <details className="group relative">
            ...
            <div className="fixed inset-x-0 top-[106px] z-50 border-y border-white/10 bg-[#0b0f15] p-4 shadow-2xl sm:p-6">
          ...
          <details className="relative lg:hidden">
            ...
            <div className="absolute right-0 top-12 w-64 rounded-2xl border border-white/10 bg-[#0b0f15] p-3 shadow-2xl">
              {nav.map((item) => (
                <Link key={item.href} href={item.href} className="block border-b border-white/8 px-3 py-3 text-xs font-black uppercase tracking-[0.12em] text-white/70 last:border-b-0">
                  {item.label}
                </Link>
              ))}
              <a href="https://auradigitalfiji.com" className="mt-3 flex items-center justify-between rounded-xl bg-white px-3 py-3 text-xs font-black uppercase tracking-[0.12em] text-black">Aura Digital Fiji <ArrowUpRight size={13}/></a>
            </div>
          </details>
      ...
      <div className="border-t border-white/8 bg-[#0b0f15]/80">
        <div className="mx-auto flex max-w-[1480px] gap-2 overflow-x-auto px-4 py-3 sm:px-6 lg:px-8 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {ribbon.map(([label, href]) => (
            <Link
              key={href}
              href={href}
              className="adi-chip min-w-max transition hover:border-white/25 hover:bg-white hover:text-black"
            >
              {label}
            </Link>
          ))}
...
export function NewsFooter() {
  return (
    <footer className="border-t border-white/10 bg-[#06080c] text-white">
      <div className="adi-footer-noise">
        <div className="mx-auto max-w-[1480px] px-4 py-14 sm:px-6 lg:px-8 lg:py-20">
          <div className="grid gap-8 lg:grid-cols-[1.2fr_.8fr_.8fr_1fr]">
            <div className="adi-panel p-6 sm:p-7">
              ...
              <p className="mt-5 max-w-md text-sm leading-7 text-white/48">Clean technology reporting, Fiji business context and evidence-linked analysis for the Pacific.</p>
              <a href="https://auradigitalfiji.com" className="mt-5 inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.12em] text-white/74 hover:text-white">Visit Aura Digital Fiji <ArrowUpRight size={13}/></a>
            </div>
            <div>
